    ...
    while True:
        ...
        if orders_list:
            ...
            for order in orders_list:
                ...
                set_event(e)
                client_msg = enter_a_new_order(symbol=mySymbol, side=mySide, price=best_price, qty=myQuantity)
                send_msg(client_msg)
                wait_for_an_event(e)
                data = get_data(q)
...
def real_trade():
    ...
    cumret = cumret.replace(np.inf, np.nan)
    cumret = cumret.replace(-np.inf, np.nan)
    result_df.loc[0,"Sharpe"] = np.sqrt(252) * np.nanmean(cumret) / np.nanstd(cumret)
    result_df = result_df.round(2)
